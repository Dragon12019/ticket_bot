...
async def add_ticket(interaction: discord.Interaction, name: str, color: Literal["grey","green","red","primary"], name_ticket_room: str, log: discord.TextChannel, role: discord.Role, emoji_ticket: str, category: discord.CategoryChannel, image: discord.Attachment = None):
    ...
    if name in data:
        ...
        color_ticket = {"color": color}
        name_ticket = {"name": name_ticket_room}
        log_ticket = {"log": log.id}
        role_ticket = {"role": role.id}
        emoji_ticket = {"emoji": str(emoji_ticket)}
        category_ticket = {"category": category.id}
        if image and image.filename.lower().endswith(image_extension):
            image_ticket = {"image": image.url}
            data[name] = [color_ticket, name_ticket, log_ticket, role_ticket, emoji_ticket, category_ticket, image_ticket]
            reply_add = "تم اضافة التذكرة الى قائمة التذاكر"
        else:
            reply_add = "لا يمكن حفظ ملفات غير الصور لكن تم حفظ باقي بيانات التذكرة لأضافة الصورة التي تريدها يرحى مسح التذكرة واضافة البيانات الخاصة بها مع الصورة التي تريدها"
            data[name] = [color_ticket, name_ticket, log_ticket, role_ticket, emoji_ticket, category_ticket]
...
async def ticket_setup(interaction: discord.Interaction, descreption: str, channel: discord.TextChannel = None, image: discord.Attachment = None):
    ...
    if not data:
        await interaction.response.send_message("لا يوجد اي تذاكر لأضافتها", ephemeral=True)
        return
    for ticket_name, ticket_data in data.items():
        role_ticket = interaction.guild.get_role(ticket_data[3]["role"])
        button = discord.ui.Button(label=ticket_name, style=colors[ticket_data[0]["color"]], emoji=ticket_data[4]["emoji"])
        ...
        async def button_callback(interaction: discord.Interaction, ticket_name=ticket_name, ticket_data=ticket_data, role_ticket=role_ticket):
            ...
            category_ticket_id = ticket_data[5]["category"]
            ...
            if category:
                channel_ticket = await category.create_text_channel(name=f"{ticket_data[1]['name']} ticket for {user}", overwrites=overwrite)
            else:
                channel_ticket = await interaction.guild.create_text_channel(name=f"{ticket_data[1]['name']} for {user}", overwrites=overwrite)
            ...
            if "image" in ticket_data[-1]:
                embed_ticket.set_image(
                    url=ticket_data[-1]["image"]
                )
            else:
                pass
            ...
            async def delete_callback(interaction: discord.Interaction):
                log_channel = interaction.guild.get_channel(ticket_data[2]["log"])
                if role_ticket in interaction.user.roles:
                    if log_channel:
...
async def add(interaction: discord.Interaction, member: discord.User):
    ...
    for roles in data.values():
        role = interaction.guild.get_role(roles[3]["role"])
